  ...
  with(as.list(c(State, Pars)), {
    ...
    if(TSUM < FINTSUM){
      ...
      RATES <- c(RROOTD = RROOTD, RWA = RWA,
                 RTSUM = RTSUM, RTRAIN = RTRAIN,  RDTR = RDTR,  RPAR = RPAR,
                 RLAI = RLAI,
      ...
      AUX <- c(TRANRF = TRANRF, 
               RUNOFF = DRUNIR$RUNOFF, DRAIN = DRUNIR$DRAIN,
               GRKGHA = (RWSO + RWLV + RWST + RWRT) * 10,
               BIOMTHA = (WSO + WLV + WST + WRT) * 0.01,
               WSOTHA = WSO * 0.01, 
               HI = WSO /(WSO + WLV + WST + WRT),
               WUE = WSO / (TRAN + EVAP) ) #g/m2 * m2/l = g/l == kg/m3
    }
    else{
      #all plant related rates are set to 0
      RATES <- c(RROOTD = 0, RWA = 0,
                 RTSUM = 0, RTRAIN = 0,  RDTR= RDTR, RPAR = RPAR,
                 RLAI = 0,
      ...
      AUX <- c(TRANRF = NA,
               ...
               WUE = ifelse((TRAN + EVAP)==0, 0, WSO /(TRAN + EVAP)))
    }
